...
@sections_list = qw(0 1 1.01 1.1 1.2 2.1 2.2 3 3.1 3.2 4 4.1 4.2 5 5.1 5.2 6) unless $ARGV[0];

for my $section (@sections_list) { # actually chapters
	# print "\\newpage\n" unless $section == 1;
	open my $tmp_fh, ">:utf8", "body_src/$section.tex" or die $!;
	select $tmp_fh; # set default for print
	print STDERR "generating chapter $section...\n";
	
	print '\chapter{' . $section . ' '
		. $dbh->selectrow_array(qq#SELECT chaptertitle FROM `chapters` WHERE `chapter` LIKE '1.$section'#)
		. "}\n\n";
	...
	for my $noteid (@{$dbh->selectcol_arrayref("SELECT noteid FROM notes WHERE "
			. "spec='C' AND id LIKE '1.$section' AND notetype='G'")}) {
	...
	for my $note (@{$dbh->selectcol_arrayref(
		"SELECT xmlnote FROM notes WHERE spec='C' AND id LIKE '1.$section' AND notetype = 'T' ORDER BY ord")}) {
...
EndOfSQL
	...
	my $chapter_end_notes = $dbh->selectcol_arrayref(
		"SELECT xmlnote FROM notes WHERE spec='C' AND id LIKE '1.$section' AND notetype = 'F' ORDER BY ord");
