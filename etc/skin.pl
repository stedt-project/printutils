...
my @sections_list = qw(6.5);

for my $section (@sections_list) { # chapter sections
	# print "\\newpage\n" unless $section == 1;
	open my $tmp_fh, ">:utf8", "skin_src/$section.tex" or die $!;
...
# 	}
	...
	foreach (@$etyma_in_section) {
		...
		$protoform =~ s/⪤} +/⪤} */g;
		$protoform =~ s/\\textasciitilde\\ +/textasciitilde\\ */g;
		$protoform =~ s/ = +/ = */g;
		$protoform = '*' . $protoform;
		...
		print "\\vspace{1em}\n";
		print "\\addcontentsline{toc}{section}{($printseq) $plg \\textbf{$protoform} $protogloss}\n"; # [\\#$tag] tmp_
...
#		print "\\markright{($printseq) $plg $protoform $protogloss}\n";
		print "\\markright{($printseq) [\\#$tag] $plg $protoform $protogloss}\n";
		
		$protoform = prettify_protoform($protoform); # make vertical

		print "{\\large\n\\parindent=-1em\n";
		print "($printseq) \\hspace{\\stretch{1}}"; # tag
		print "$plg $protoform";
