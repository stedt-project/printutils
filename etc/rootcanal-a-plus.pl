...
# 2007.06.15
# ----------
# See rootcanal-a.pl. This is just to extract extra Bisu forms.
...
CATCH	557	da
CLASSIFIER (general)	951,949,955,953, 954, 952, 948		má	mà
...
DUMB/MUTE	302		qā		ʔa'
ELDER BROTHER %BROTHER	333		à-ka			Mand. gē
	...
	for my $ref (split /, */, $refstr) {
		...
		my $maingloss = ''; # one main gloss for each ref
			...
			my $sql;
				...
				$sql = <<EndOfSQL;
SELECT DISTINCT lexicon.rn, analysis, reflex, gloss, notes.rn, languagenames.srcabbr, lexicon.srcid
  FROM lexicon, languagenames
  	LEFT JOIN notes ON notes.rn=lexicon.rn
  WHERE  languagenames.language = 'Bisu'
    AND  ($glossSearch)
...
EndOfSQL
		...
		print "\\begin{tabular}{p{3in}c}\n";

			my @notes;
			my $lastform;
			for my $rec (@$recs) {
				my ($rn,$an,$form,$gloss,$notern, $srcabbr, $srcid) = @$rec;
				if (!defined($lastform) || $form ne $lastform) {
					...
					$an = "($an)" if $an;
					$srcabbr .= ":$srcid" if $srcid;
					print escape_tex($form) . " $an [$rn] $srcabbr ";
					print "‘" . escape_tex($gloss) . "’";
